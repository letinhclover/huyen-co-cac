...
const EMPTY_TOPIC_STATE: TopicState = { loadState: "idle", text: "", error: null };
...
export function FortuneCard({ date, userProfile, onSetupProfile }: FortuneCardProps) {
  ...
  // State cho "Tổng quan" (auto-load)
  const [overviewState, setOverviewState] = useState<TopicState>(EMPTY_TOPIC_STATE);
  // State cho topic đang được chọn (ngoài Tổng quan)
  const [activeTopic, setActiveTopic]     = useState<FortuneTopic | null>(null);
  const [topicState, setTopicState]       = useState<TopicState>(EMPTY_TOPIC_STATE);
  ...
  // ── Khi date / profile thay đổi: reset toàn bộ, auto-load Tổng quan ──
  useEffect(() => {
    setActiveTopic(null);
    setTopicState(EMPTY_TOPIC_STATE);

    if (!userProfile) {
      setOverviewState(EMPTY_TOPIC_STATE);
      return;
    }
    ...
    if (cached) {
      // Hiện cached ngay, không typewriter lại
  ...
  }, [dateIso, userProfile?.birthYear]);

  const handleOverviewTypingDone = useCallback(() => {
    if (!userProfile) return;
    const key = makeCacheKey(dateIso, userProfile.birthYear, "Tổng quan");
    setOverviewState((prev) => {
      setCachedFortune(key, { text: prev.text, topic: "Tổng quan", generatedAt: new Date().toISOString(), cached: false });
      return { ...prev, loadState: "done" };
    });
  }, [dateIso, userProfile]);

  // ── Bấm topic (Sự Nghiệp / Tình Duyên / Tài Lộc) ──
  const handleTopicClick = useCallback(async (topic: FortuneTopic) => {
    ...
    // Toggle off nếu bấm lại topic đang xem
  ...
  const handleTopicTypingDone = useCallback(() => {
    if (!userProfile || !activeTopic) return;
    const key = makeCacheKey(dateIso, userProfile.birthYear, activeTopic);
    setTopicState((prev) => {
      setCachedFortune(key, { text: prev.text, topic: activeTopic, generatedAt: new Date().toISOString(), cached: false });
      return { ...prev, loadState: "done" };
    });
  }, [dateIso, userProfile, activeTopic]);
  ...
  const handleShare = useCallback(async () => {
    ...
    setIsSharing(true);
    try {
      const filename = `huyen-co-cac-${dateIso}.png`;
      await captureAndShare(shareRef.current, filename);
    } catch {
      /* ignore */
    } finally {
      setIsSharing(false);
  ...
  const handleRetryOverview = useCallback(() => {
    ...
    generateDailyFortune(
      ...
      .then((r) => setOverviewState({ loadState: "typing", text: r.text, error: null }))
      .catch((e: GeminiError) => setOverviewState({ loadState: "error", text: "", error: e }));
  }, [userProfile, todayCanChi, dateLabel]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4">
          ...
          <AnimatePresence mode="wait">
            {overviewState.loadState === "loading" && (
              <motion.div key="ov-loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            {(overviewState.loadState === "typing" || overviewState.loadState === "done") && overviewState.text && (
              <motion.div key="ov-text" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <TypewriterBlock
                  text={overviewState.text}
                  isTyping={overviewState.loadState === "typing"}
                  speed={30}
                  onDone={handleOverviewTypingDone}
                />
              </motion.div>
            )}
        ...
        {/* ── Topic result ── */}
        <AnimatePresence>
          {activeTopic && (topicState.loadState === "loading" || topicState.loadState === "typing" || topicState.loadState === "done" || topicState.loadState === "error") && (
            <motion.div
              key={activeTopic}
                ...
                <AnimatePresence mode="wait">
                  ...
                  {(topicState.loadState === "typing" || topicState.loadState === "done") && topicState.text && (
                    <motion.div key="tp-text" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <TypewriterBlock
                        text={topicState.text}
                        isTyping={topicState.loadState === "typing"}
                        speed={28}
                        onDone={handleTopicTypingDone}
                      />
                    </motion.div>
                  )}
                  ...
                  {topicState.loadState === "error" && topicState.error && (
                    <motion.div key="tp-error" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <ErrorBlock error={topicState.error} onRetry={() => activeTopic && handleTopicClick(activeTopic)} small />
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        <div className="px-5 pb-3 flex items-center justify-between">
          <p className="text-white/12 text-[10px] tracking-wide">
            🔮 Thỉnh quẻ tại: huyen-co-cac.pages.dev
          </p>
          <p className="text-white/12 text-[10px]">{dateLabel}</p>
        </div>
      </div>

      {/* ── Share button (ngoài vùng capture) ── */}
      <AnimatePresence>
        {overviewDone && (
          <motion.div
...
function TypewriterBlock({ text, isTyping, speed = 30, onDone }: TypewriterBlockProps) {
  const { displayed, done } = useTypewriter(isTyping ? text : "", speed, onDone);
  const shownText = isTyping ? displayed : text;
...
function ErrorBlock({ error, onRetry, small }: ErrorBlockProps) {
  const ICONS: Record<string, string> = {
    no_api_key: "🔑", network: "📡", rate_limit: "⏳", unknown: "😅",
  };
  const retryable = error.type !== "no_api_key";
